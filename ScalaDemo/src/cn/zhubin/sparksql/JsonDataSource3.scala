...
object JsonDataSource3 {
  def main(args:Array[String]):Unit={
    ...
    val goodStudentScoreDF =  sqlContext.sql("select name,score form student_score where score >= 80")
    ...
    sql += ")"
    val goodStuentInfoDF =  sqlContext.sql(sql)

    val goodStudentRDD =  goodStudentScoreDF.rdd.map(row => (row.getAs[String]("name"),row.getAs[Long]("score")))
      .join(goodStuentInfoDF.rdd.map(row => (row.getAs[String]("name"),row.getAs[Long]("age"))))
